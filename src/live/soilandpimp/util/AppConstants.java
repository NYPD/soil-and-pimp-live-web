package live.soilandpimp.util;

public class AppConstants {

    public static final String PROJECT_NAME = "soil-and-pimp-live-web";
    public static final String APPLICATION_NAME = "Soil and Pimp Live Web";
    public static final String DEVELOPMENT_PROFILE = "DEVELOPMENT";
    public static final String TEST_PROFILE = "TEST";
    public static final String PRODUCTION_PROFILE = "PRODUCTION";
    public static final String CASSANDRA_KEYSPACE = "soilandpimp";

    public static final String REMEBER_ME_PARAMETER_NAME = "remember_me";
    public static final String API_TYPE_COOKIE_NAME = "apiType";

    public static final int FIVE_YEARS_IN_SECONDS = 157680000;

}
